...
up‑aggregation designs** you can toggle at runtime.  This version patches the
`bridge_type="attention"` path so that key/value and query dimensions align and
soft‑max scores stay numerically stable.

| Bridge | Flag | Default | How it works | Pros | Cons |
|--------|------|---------|--------------|------|------|
| **MLP** | "mlp" | ✅ | Concat → 2‑layer MLP → AE enc | Cheap & stable | Linear mix only |
| **Hier‑AE** | "hier_ae" |   | Concat → bridge‑AE → slow AE | True compression | Extra params |
| **Cross‑Attention** | "attention" |   | Q = x ; K/V = lower latents | Content‑routed | Most compute |

Defaults remain: tags ✔︎, contrastive ✔︎, critic ✘, `bridge_type='mlp'`.
"""
...
class MemoryRung(nn.Module):
    def __init__(self, name, tau, in_d, lat_d, hid, cfg: CLConfig, slots=1):
        ...
        # bridge modules
        bridge_in_dim = in_d + cfg.tag_dim + lat_d * slots  # x + tag + lower
        if cfg.bridge_type == "mlp":
            ...
            # **HOT‑FIX** – project keys/values to same dim as queries
    ...
    def _bridge_process(self, x, lower_lat):
        ...
        if self.cfg.bridge_type == "attention" and lower_lat:
            # project & normalise keys/values and query to avoid soft‑max blow‑up
            #K_raw = torch.cat(lower_lat).unsqueeze(0) if len(lower_lat) == 1 else torch.stack(lower_lat)  # n × ?
            K_raw = torch.stack(lower_lat)
            K = F.normalize(self.k_proj(K_raw), dim=-1)  # n × lat_d
            q = F.normalize(self.q_proj(x), dim=-1)       # lat_d
            #attn = (K @ q) / math.sqrt(K.size(-1))        # n
            attn = K @ q 
            w = attn.softmax(0)
            context = (w.unsqueeze(-1) * K).sum(0)
            return torch.cat([x, context, self.tag], -1)
        # if no lower latents yet (first rung) or not using attention
...
class ChronoLadderLM(nn.Module):
    def __init__(self, cfg: CLConfig | None = None, backbone="gpt2-medium"):
        ...
        fused = sum(r.latents.numel() + self.cfg.tag_dim for r in self.rungs)    # (latent slots + tag) 
